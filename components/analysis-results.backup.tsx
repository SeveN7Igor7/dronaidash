...
export function AnalysisResults({ data }: AnalysisResultsProps) {
  ...
  return (
    <div className="space-y-6">
      {/* NOVO TOPO: resumo visual com kpis */}
      <div className="mx-4 sm:mx-0">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Resultados da Análise</h1>
          <p className="text-slate-500">Resumo e recomendações geradas em: {new Date(metadata.processingDate || Date.now()).toLocaleString()}</p>
        </div>

        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard title="Vegetação" value={`${safeToFixed(spectralAnalysis?.landCover?.vegetation, 0)}%`} icon={<Leaf className="h-4 w-4" />} color="green" />
          <StatCard title="Área Urbana" value={`${safeToFixed(spectralAnalysis?.landCover?.urban, 0)}%`} icon={<MapPin className="h-4 w-4" />} color="rose" />
          <StatCard title="Água" value={`${safeToFixed(spectralAnalysis?.landCover?.water, 0)}%`} icon={<Droplet className="h-4 w-4" />} color="blue" />
          <StatCard title="Saúde (NDVI)" value={`${safeToFixed(spectralAnalysis?.ndvi?.mean, 2)}`} icon={<TrendingUp className="h-4 w-4" />} color="green" />
        </div>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <Card>
            <CardHeader>
              <CardTitle>📊 Índices Espectrais Rápidos</CardTitle>
              <CardDescription className="text-sm">Visão rápida de NDVI, EVI e SAVI</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                <MetricBar label="NDVI (média)" value={Math.round(((spectralAnalysis?.ndvi?.mean || 0) + 1) * 50)} color="green" />
                <MetricBar label="EVI (média)" value={Math.round(((spectralAnalysis?.evi?.mean || 0) + 1) * 50)} color="blue" />
                <MetricBar label="SAVI (média)" value={Math.round(((spectralAnalysis?.savi?.mean || 0) + 1) * 50)} color="green" />
              </div>
            </CardContent>
          </Card>
        </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <Badge>{safeToFixed((patterns as any)?.geometricPatterns, 1)}%</Badge>
                    </div>
                    <div className="flex justify-between">
                      <span>Detecção de Bordas</span>
                      <Badge>{safeToFixed((patterns as any)?.edgeDetection, 1)}%</Badge>
                    </div>
                    <div className="flex justify-between">
                      <span>Variação de Cor</span>
                      <Badge>{safeToFixed((patterns as any)?.colorVariation, 1)}%</Badge>
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <Badge variant={(areaClassification?.scores?.urban || 0) > 50 ? "destructive" : "outline"}>
                        {areaClassification?.scores?.urban || 0}/100
                      </Badge>
                    </div>
                    <div className="flex justify-between">
                      <span>🌾 Pontuação Rural</span>
                      <Badge variant={(areaClassification?.scores?.rural || 0) > 50 ? "default" : "outline"}>
